...
class Agent():
    ...
    def get_actions_epsilon(self, obs: np.ndarray) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    with logging_redirect_tqdm():
        for step in range(config["n_update_steps"]):
            actions_array = np.zeros((batch_size, n_envs))
            obs_array = np.zeros((batch_size, n_envs, config["in_featues"]))
            rewards_array = np.zeros((batch_size, n_envs))
            done_mask = np.zeros((batch_size, n_envs))
            for index in range(config["batch_size"]):
                actions = agent.get_actions_epsilon(obs=obs)
                actions_array[index] = actions
                obs_array[index] = obs
                obs, rewards, termindated, truncated, infos = envs_wrapper.step(actions)
                done = [ter or tru for ter, tru in zip(termindated, truncated)]
                rewards_array[index] = rewards
                done_mask[index] = done
            ...
            actions_array = rearrange(actions_array, "b n -> n b")
            obs_array = rearrange(obs_array, "b n f -> n b f")
            rewards_array = rearrange(rewards_array, "b n -> n b")
            done_mask = rearrange(done_mask, "b n -> n b")
            
            actions_log_probs = agent.get_log_prob(obs=obs_array, actions=actions_array)
            loss = agent.get_vanilla_loss(actions_log_probs=actions_log_probs, rewards=rewards_array, masks=done_mask)
            
            agent.update_actor(loss=loss)
